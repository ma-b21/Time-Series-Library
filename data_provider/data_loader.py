...
class Dataset_Futures(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='futures_product1.csv',
                 target='close_price', scale=True, timeenc=0, freq='t', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
        self.args = args
        # info
        if size == None:
    ...
    def __read_data__(self):
        ...
        '''
        df_raw.columns: ['date', feature1, feature2, ..., target, factor1, factor2, ...]
        根据 args.enc_in 决定使用前多少列作为主特征
        剩余列作为辅助特征（类似时间特征）
        '''
        cols = list(df_raw.columns)
        cols.remove('date')
        ...
        # 重新排列：date 在最前，其他特征按原顺序，target 在指定位置
        df_raw = df_raw[['date'] + cols + [self.target]]
        ...
        # 获取所有特征列（除了date和target）
        all_feature_cols = [col for col in df_raw.columns if col not in ['date', self.target]]
        
        # 根据 features 和 enc_in 选择特征
        if self.features == 'M':
